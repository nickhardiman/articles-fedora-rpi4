...
!!! I'm using firmware in a new and possibly misleading way here, as all software that runs before the OS runs. 
That's the view of a mobile phone user - it's the software you don't care about.  
The traditional view does not label everything between hardware and software as firmware. 
Software embedded in a piece of hardware is firmware. 
A CPU doesn't run it, it's hard to update, and invisible to users. 
Then there's the middle ground view that says UEFI is firmware, bootloader is not. 
